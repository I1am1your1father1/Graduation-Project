...
def solve_one_dataset(dataset):
    ...
    start = time.time()
    colors = dsatur_coloring(adj, degree)
...
def parse_args():
    parser = argparse.ArgumentParser(description="DSATUR baseline for Graph Coloring")
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--save_name",
        type=str,
        default="coloring_dsatur_results.csv",
        help="保存的 csv 文件名",
    )
