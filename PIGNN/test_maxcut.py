...
from utils import (
    generate_graph,
    get_gnn,
    run_gnn_training_maxcut,
    qubo_dict_to_torch,
    gen_combinations,
    loss_func_maxcut,
    gen_q_dict_maxcut,
    postprocess_maxcut_bitstring,
)
...
# =========================
# 5. 单次运行 PI-GNN 求解 MaxCut
# =========================

def run_one_pignn_maxcut_trial(dataset: Datasets, run_id: int, seed: int):
    """
    对一个数据集运行一次 PI-GNN 求解 MaxCut。
    """
    set_seed(seed)
    ...
    # 5. 运行 PI-GNN MaxCut
    print("Running PI-GNN for MaxCut...")
    start_time = time()

    _, epoch, final_bitstring, best_bitstring = run_gnn_training_maxcut(
        q_torch,
        graph_dgl,
        net,
        embed,
    ...
    # 6. 处理最优 bitstring
    cut_value, cut_ratio, partition_0, partition_1 = postprocess_maxcut_bitstring(
        best_bitstring,
        nx_graph,
    )
    ...
    # 8. 计算 best loss
    try:
        best_loss = loss_func_maxcut(
            best_bitstring.float().to(TORCH_DEVICE),
            q_torch,
        )
        best_loss_value = float(best_loss.detach().cpu().item())
    except Exception:
        best_loss_value = None

    print(
        f"PI-GNN MaxCut result | "
        f"Cut value: {cut_value} | "
        f"Cut ratio: {cut_ratio:.6f} | "
        f"|S0|: {len(partition_0)} | "
        f"|S1|: {len(partition_1)} | "
        f"Epoch: {epoch} | "
    ...
    result = {
        ...
        "num_nodes": n,
        "num_edges": m,
        "maxcut_value": int(cut_value),
        "cut_ratio": float(cut_ratio),
        "partition_0_size": int(len(partition_0)),
        "partition_1_size": int(len(partition_1)),
        "epoch": int(epoch),
...
def summarize_dataset_results(dataset: Datasets, dataset_results):
    """
    汇总一个数据集 10 次 MaxCut 运行结果。

    best_maxcut_value:
        10 次运行中最大的 cut value。
    ...
    # 去掉失败的运行
    df = df[df["maxcut_value"].notna()].copy()
    ...
    if len(df) == 0:
        return {
            ...
            "num_nodes": None,
            "num_edges": None,
            "best_maxcut_value": None,
            "best_run": None,
            "best_seed": None,
            "best_cut_ratio": None,
            "avg_maxcut_value": None,
            "std_maxcut_value": None,
            "avg_cut_ratio": None,
            "std_cut_ratio": None,
            "avg_time_sec": None,
            "std_time_sec": None,
    ...
    best_row = df.loc[df["maxcut_value"].idxmax()]
    ...
    summary = {
        ...
        # 10 次中最好结果
        "best_maxcut_value": int(best_row["maxcut_value"]),
        "best_run": int(best_row["run"]),
        "best_seed": int(best_row["seed"]),
        "best_cut_ratio": float(best_row["cut_ratio"]),

        # 所有运行的平均结果
        "avg_maxcut_value": float(df["maxcut_value"].mean()),
        "std_maxcut_value": float(df["maxcut_value"].std(ddof=0)),
        "avg_cut_ratio": float(df["cut_ratio"].mean()),
        "std_cut_ratio": float(df["cut_ratio"].std(ddof=0)),
...
def main():
    ...
    for dataset_idx, dataset in enumerate(ALL_DATASETS):
        ...
        for run_id in range(NUM_RUNS):
            ...
            try:
                result = run_one_pignn_maxcut_trial(
                    dataset=dataset,
                    run_id=run_id,
                    seed=seed,
                )
                ...
                result = {
                    ...
                    "maxcut_value": None,
                    "cut_ratio": None,
                    "partition_0_size": None,
                    "partition_1_size": None,
                    "epoch": None,
                    "time_sec": None,
                    "final_loss": None,
                    "best_loss": None,
        ...
        print(
            f"Best MaxCut value: {summary['best_maxcut_value']} | "
            f"Average MaxCut value: {summary['avg_maxcut_value']} ± {summary['std_maxcut_value']} | "
            f"Average cut ratio: {summary['avg_cut_ratio']} | "
    ...
    detail_path = save_dir / "maxcut_pignn_run_details.csv"
    summary_path = save_dir / "maxcut_pignn_summary.csv"
